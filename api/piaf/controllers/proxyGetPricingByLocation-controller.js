...
const getPricingLite_adapter_1 = require("../adapters/getPricingLite-adapter");
const flatten = (object) => {
    return Object.assign({}, ...(function _flatten(objectBit, path = "") {
        return [].concat(...Object.keys(objectBit).map((key) => typeof objectBit[key] === "object"
            ? _flatten(objectBit[key], `${path}/${key}`)
            : { [`${path}/${key}`]: objectBit[key] }));
    })(object));
};
const proxyGetPricingByLocation = (req, res) => {
    var body = req.body || {};
    var test = {
    ...
    axios(authSF).then(function (auth) {
        ...
        var callSF = {
            method: "post",
            url: "https://prometheusreg--training.sandbox.my.salesforce.com/services/apexrest/getNeighborhoodsByLocation",
            headers: {
        ...
        axios(callSF)
            ...
            if (response.data.length > 0) {
                ...
                getPricingLite_adapter_1.GetPricingLiteAdapter.getPricing(inputToRP).then((output) => {
...
//# sourceMappingURL=proxyGetPricingByLocation-controller.js.map